    ...
    Cell.prototype.render = function (cell) {
        ...
        if (this.type === '$_split_') {
            ...
            this.outputPorts.forEach(function (op, i) {
                ...
                var portX = Number(outPorts_1[1][1]['s:x']);
                var portY = startY_1 + i * gap_1;
                portClone[portClone.length - 1][2] = op.Key;
                portClone[1].transform = 'translate(' + portX + ','
                    + portY + ')';
                portClone[1]['s:x'] = String(portX);
                portClone[1]['s:y'] = String(portY);
            ...
            this.inputPorts.forEach(function (port, i) {
                ...
                var portX = Number(inPorts_1[1][1]['s:x']);
                var portY = startY_2 + i * gap_2;
            ...
            var cellPortById_1 = {};
            (cell.ports || []).forEach(function (portInfo) {
                cellPortById_1[portInfo.id] = portInfo;
            });
            var orderedInputPorts_1 = this.inputPorts.slice().sort(function (a, b) {
                var aInfo = cellPortById_1[a.parentNode.Key + '.' + a.Key];
                var bInfo = cellPortById_1[b.parentNode.Key + '.' + b.Key];
                var aY = aInfo ? aInfo.y : 0;
                var bY = bInfo ? bInfo.y : 0;
                return aY - bY;
            });
            var orderedOutputPorts_1 = this.outputPorts.slice().sort(function (a, b) {
                var aInfo = cellPortById_1[a.parentNode.Key + '.' + a.Key];
                var bInfo = cellPortById_1[b.parentNode.Key + '.' + b.Key];
                var aY = aInfo ? aInfo.y : 0;
                var bY = bInfo ? bInfo.y : 0;
            ...
            tempclone.pop();
            tempclone.pop();
            orderedInputPorts_1.forEach(function (port, i) {
                var portClone = clone(inPorts_2[0]);
                var portInfo = cellPortById_1[port.parentNode.Key + '.' + port.Key];
                var portX = portInfo ? portInfo.x : Number(inPorts_2[1][1]['s:x']);
                var portY = portInfo ? portInfo.y : instartY_1 + i * ingap_1 + shifts_2.inShift;
                portClone[portClone.length - 1][2] = port.Key;
                portClone[1].transform = 'translate(' + portX + ','
                    + portY + ')';
                portClone[1]['s:x'] = String(portX);
            ...
            orderedOutputPorts_1.forEach(function (port, i) {
                var portClone = clone(outPorts_2[0]);
                var portInfo = cellPortById_1[port.parentNode.Key + '.' + port.Key];
                var portX = portInfo ? portInfo.x : Number(outPorts_2[1][1]['s:x']);
                var portY = portInfo ? portInfo.y : outstartY_1 + i * outgap_1 + shifts_2.outShift;
