...
function createFlatModule(skinData, yosysNetlist) {
    ...
    if (layoutProps.splitsAndJoins !== false) {
        try {
            ...
            // Large sequential graphs such as register files can trigger
            // deep recursion in netlistsvg's split/join gather step.
            // Skip the optional split/join enrichment and continue rendering.
            console.log('[WARN] netlistsvg addSplitsJoins skipped:', err && err.message ? err.message : err);
        }
    }
...
function render(skinData, yosysNetlist, done, elkData) {
    ...
    if (elkData) {
        promise = new Promise(function (resolve) {
            var output = drawModule_1.default(elkData, flatModule);
            resolve(output);
        });
    }
    else {
