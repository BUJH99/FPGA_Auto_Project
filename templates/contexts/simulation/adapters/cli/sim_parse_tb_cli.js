...
function usageAndExit() {
    console.error('Usage: node tools/parse_tb.js <path_to_tb_file.(v|sv)> [project_root] --manifest-json <path>');
    process.exit(1);
}
...
if (!tbFileArg) usageAndExit();

const tbFile = path.resolve(tbFileArg);
if (!fs.existsSync(tbFile)) {
    console.error(`Error: Testbench file not found at ${tbFile}`);
    process.exit(1);
}
const projectRootArg = projectRootRaw ? path.resolve(projectRootRaw) : null;
if (projectRootArg && !fs.existsSync(projectRootArg)) {
    console.error(`Error: Project root not found at ${projectRootArg}`);
    process.exit(1);
}
const manifestJsonPath = manifestJsonArg ? path.resolve(manifestJsonArg) : '';
if (!manifestJsonPath) {
    console.error('Error: --manifest-json is required in strict mode.');
    process.exit(1);
}
if (!fs.existsSync(manifestJsonPath)) {
    console.error(`Error: Manifest JSON not found at ${manifestJsonPath}`);
    process.exit(1);
}
...
try {
    ...
    const manifestPayload = JSON.parse(fs.readFileSync(manifestJsonPath, 'utf-8'));
    const manifestErrors = Array.isArray(manifestPayload.errors) ? manifestPayload.errors : [];
    if (manifestErrors.length > 0) {
        const codes = manifestErrors.map((e) => e.code).filter(Boolean).join(',');
        throw new Error(`manifest_has_errors:${codes || 'unknown_error'}`);
    }
    const resolved = manifestPayload.resolved || {};
    const rows = []
        .concat(Array.isArray(resolved.src_files) ? resolved.src_files : [])
        .concat(Array.isArray(resolved.tb_files) ? resolved.tb_files : []);
    for (const rel of rows) {
        const entry = String(rel || '').trim().toLowerCase();
        if (!entry) continue;
        if (entry.endsWith('.v')) hasV = true;
        if (entry.endsWith('.sv') || entry.endsWith('.svh')) hasSv = true;
    }
    if (!hasV && !hasSv) {
        throw new Error('manifest_resolved_empty:src_tb');
    }
    hdlLang = hasV && hasSv ? 'mixed' : (hasSv ? 'systemverilog' : 'verilog');
} catch (err) {
    if (String(err && err.message || '').startsWith('manifest_')) {
