...
set "VITIS_STEP=run_application"
set "NO_PAUSE=0"
set "SELECTED_APPS="
set "SELECTED_TARGET="
...
if "%~1"=="" (
    ...
    echo Usage: %~nx0 ^<Project_Directory^> [--app name] [--target hw]
    exit /b 1
)
...
if not defined SELECTED_APPS if not "%NO_PAUSE%"=="1" (
    call "%TEMPLATES_ROOT%\contexts\vitis\adapters\bat\vitis_select_helper.bat" apps "%TARGET_PROJECT%" "%MANIFEST_JSON%" 0 SELECTED_APPS
...
echo [RUN] Running Vitis application...
call vitis -s "%TEMPLATES_ROOT%\contexts\vitis\adapters\python\vitis_run_application.py" --plan "%VITIS_PLAN_JSON%" --result "%VITIS_RESULT_JSON%"
