...
proc riscv_timing_analysis::single_cycle::phase_enabled {requested_phase target_phase} {
  return [expr {$requested_phase eq "full" || $requested_phase eq $target_phase}]
}
...
proc riscv_timing_analysis::single_cycle::run {} {
  ...
  riscv_timing_analysis::ensure_default analysis_phase "full"
  ...
  set run_actual_phase [riscv_timing_analysis::single_cycle::phase_enabled $::analysis_phase "actual_only"]
  set run_hierarchical_phase [riscv_timing_analysis::single_cycle::phase_enabled $::analysis_phase "hierarchical_only"]
  if {!$run_actual_phase && !$run_hierarchical_phase} {
    error "analysis_phase must be one of `full`, `actual_only`, or `hierarchical_only`."
  }

  set total_progress_steps [expr {(($run_actual_phase ? 1 : 0) + ($run_hierarchical_phase ? 1 : 0)) * 3}]
  set progress_step 0

  if {$run_actual_phase} {
    riscv_timing_analysis::load_source_files $::source_files
    riscv_timing_analysis::emit_progress [incr progress_step] $total_progress_steps "Loaded single-cycle RTL sources"

    synth_design -top $::top_name -part $::part_name
    riscv_timing_analysis::write_clock_and_reset_constraints $::clock_port $::reset_port $::clk_period_ns
    riscv_timing_analysis::emit_progress [incr progress_step] $total_progress_steps "Completed single-cycle synthesis"
    ...
    riscv_timing_analysis::emit_progress [incr progress_step] $total_progress_steps "Completed single-cycle timing artifacts"

    close_design
    if {$run_hierarchical_phase && [llength [get_projects -quiet]] > 0} {
      close_project -quiet
    }
  }

  if {$run_hierarchical_phase} {
    riscv_timing_analysis::load_source_files $::source_files
    riscv_timing_analysis::emit_progress [incr progress_step] $total_progress_steps "Reloaded RTL sources for hierarchical analysis"
    synth_design -top $::top_name -part $::part_name -flatten_hierarchy none
    riscv_timing_analysis::write_clock_and_reset_constraints $::clock_port $::reset_port $::clk_period_ns
    riscv_timing_analysis::emit_progress [incr progress_step] $total_progress_steps "Completed hierarchical synthesis"
    ...
    riscv_timing_analysis::emit_progress [incr progress_step] $total_progress_steps "Completed hierarchical analysis artifacts"
