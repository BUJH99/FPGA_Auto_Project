...
function usageAndExit() {
    console.error('Usage: node generate_doc.js <project_root> --manifest-json <path>');
    process.exit(1);
}
...
const SOURCE_FILES = (manifestContext.srcFiles || [])
    .filter((filePath) => /\.(v|sv)$/i.test(filePath))
    .sort((a, b) => a.localeCompare(b));
const SOURCE_BY_BASENAME_LOWER = new Map();
SOURCE_FILES.forEach((absPath) => {
    const key = path.basename(absPath).toLowerCase();
    if (!SOURCE_BY_BASENAME_LOWER.has(key)) {
        SOURCE_BY_BASENAME_LOWER.set(key, absPath);
    }
});
...
function getSubmoduleSourceLink(moduleName) {
    ...
    sourceCandidates.forEach((candidate) => {
        ...
        const hit = SOURCE_BY_BASENAME_LOWER.get(candidate.toLowerCase());
...
if (SOURCE_FILES.length === 0) {
    ...
    const fileEntries = SOURCE_FILES.map((absPath) => ({
        absPath,
    ...
    rl.question('Enter file numbers to generate docs for (e.g., "1,3,5" or "all"): ', (answer) => {
        ...
        if (input === 'all' || input === '') {
            ...
            selectedEntries = fileEntries.filter((_, i) => indices.includes(i + 1));
        }
        ...
        if (selectedEntries.length === 0) {
            ...
            selectedEntries.forEach((entry) => {
                results.push(generateMarkdown(entry.absPath));
            });
