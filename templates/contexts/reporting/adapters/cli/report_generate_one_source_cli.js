...
let hdlIndexer = null;
try {
    hdlIndexer = require('../../../code_intel/adapters/cli/code_index_hdl_cli.js');
} catch {
    hdlIndexer = null;
}
...
const sourceFilesAbs = (manifestContext.srcFiles || [])
    .filter((filePath) => /\.(v|sv)$/i.test(filePath))
    .sort((a, b) => a.localeCompare(b));
const tbFilesAbs = (manifestContext.tbFiles || [])
    .filter((filePath) => /\.(v|sv)$/i.test(filePath))
    .sort((a, b) => a.localeCompare(b));
...
function readVerilogModules() {
    if (sourceFilesAbs.length === 0) {
        throw new Error('manifest_resolved_empty:src_files');
    }

    const modules = sourceFilesAbs.map(absPath => {
...
function collectWaveformPaths(moduleName) {
    ...
    const matchedTbPath = tbFilesAbs.find((absPath) => {
    ...
    const tbBaseName = matchedTbPath
        ...
        : `tb_${moduleName}`;
    const expectedTb = matchedTbPath;
    const tbLogDir = expectedTb ? path.dirname(expectedTb) : projectRoot;
