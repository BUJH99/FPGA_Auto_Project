...
set asset_dir [file normalize [file join $templates_root "contexts" "reporting" "assets"]]
...
set modules_list {}
set src_files {}
if {$src_list_file ne "" && [file exists $src_list_file]} {
    set src_fp [open $src_list_file r]
    while {[gets $src_fp line] >= 0} {
        set rel_path [string trim $line]
        if {$rel_path eq ""} {
            continue
        }
        set abs_path [file normalize [file join $project_root $rel_path]]
        if {![file exists $abs_path]} {
            puts "\[WARNING\] Manifest src file not found: $abs_path"
            continue
        }
        if {[file isdirectory $abs_path]} {
            continue
        }
        if {![regexp -nocase {\.v$|\.sv$} $abs_path]} {
            continue
        }
        lappend src_files $abs_path
    }
    close $src_fp
    set src_files [lsort -dictionary -unique $src_files]
} else {
    puts "\[WARNING\] src list file not provided or not found: $src_list_file"
}
