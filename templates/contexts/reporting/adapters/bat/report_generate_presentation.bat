...
set "TEMPLATE_DIR=%TEMPLATES_ROOT%\contexts\reporting\presentation"
...
if "!DESIGN_INPUT!"=="1" (
    set "TEMPLATE_FILE=%TEMPLATE_DIR%\presentation_template_1.html"
    echo [INFO] Design 1 selected: presentation_template_1.html
) else if "!DESIGN_INPUT!"=="2" (
    set "TEMPLATE_FILE=%TEMPLATE_DIR%\presentation_template_2.html"
    echo [INFO] Design 2 selected: presentation_template_2.html
) else (
...
rem ---- Run Generator -------------------------------------------------------
%PY_CMD% "%PY_TOOL%" --project "%TARGET_PROJECT%" --manifest-json "%MANIFEST_JSON%" --template "!TEMPLATE_FILE!" !CLEAN_ARG! !TOP_TB_ARG! !AUTO_TB_ARG!
if errorlevel 1 (
    echo.
