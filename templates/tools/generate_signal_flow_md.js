...
function usage() {
  console.log(
    "Usage: node templates/tools/generate_signal_flow_md.js --project <path> [--signal <name>] [--top <module>] [--depth <n|MAX>] [--hier-depth <n>] [--out <path>] [--list-signals] [--list-signals-raw]"
  );
}
...
function listVerilogFiles(srcDir) {
  const out = [];
  const files = fs.readdirSync(srcDir, { withFileTypes: true });
  for (const file of files) {
    if (!file.isFile()) continue;
    if (!/\.s?v$/i.test(file.name)) continue;
...
function buildModuleDatabase(srcDir) {
  const warnings = [];
  const files = listVerilogFiles(srcDir);
  const moduleList = [];
...
function main() {
  ...
  const db = buildModuleDatabase(srcDir);
  if (db.modulesByName.size === 0) {
    console.error("[ERROR] No Verilog modules were parsed from src.");
    process.exit(1);
  }
