...
if (args.length < 1) {
    console.error('Usage: node annotate_hdl_info.js <ProjectDir> [--include-legacy]');
    process.exit(1);
}
...
if (!fs.existsSync(srcDir)) {
    console.error(`[ERROR] Missing src directory: ${srcDir}`);
    process.exit(1);
}
if (!fs.existsSync(tbDir)) {
    console.error(`[ERROR] Missing tb directory: ${tbDir}`);
    process.exit(1);
}
...
function main() {
    const srcFiles = walkVerilogFiles(srcDir);
    const tbFiles = walkVerilogFiles(tbDir, { skipLegacy: !includeLegacy });
