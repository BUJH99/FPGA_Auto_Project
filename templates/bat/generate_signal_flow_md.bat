...
set "SCRIPT_DIR=%~dp0"
set "TOOL_SCRIPT=%SCRIPT_DIR%..\tools\generate_signal_flow_md.js"
...
for /f "delims=" %%F in ('dir /b /a-d src\*.v src\*.sv 2^>nul') do (
...
if !MODULE_COUNT! equ 0 (
    echo [ERROR] No Verilog source files found in src.
    if not defined NO_PAUSE pause
    exit /b 1
)
...
node "%TOOL_SCRIPT%" --project "%TARGET_PROJECT%" --top "!TOP_MODULE!" --list-signals --list-signals-raw > "%SIG_LIST_FILE%"
if errorlevel 1 (
...
for /l %%I in (1,1,!SELECT_COUNT!) do (
    ...
    node "%TOOL_SCRIPT%" --project "%TARGET_PROJECT%" --top "!TOP_MODULE!" --signal "!CUR_SIG!" --depth "!TRACE_DEPTH!"
    if errorlevel 1 (
        echo [FAIL] !CUR_SIG!
        set /a FAIL_COUNT+=1
    ) else (
